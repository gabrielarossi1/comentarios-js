...
function renderComments(arr, parent){
    arr.forEach(element => {
        ...
        replyButton.addEventListener('click', e => {
            const newInput = inputContainer.cloneNode(true); //lo clonamos con todo
            newInput.value = '';
            newInput.focus();
        ...
        if(element.responses.length > 0){
            renderComments(element.responses, responsesContainer);  //recursividad, fuerzo un loop con un condicional que rompe el ciclo
        }
